...
실행하지 않습니다. 별도 [#63 CLI 미리보기](../guides/PROPOSAL_PREVIEW.md)는 이제 검증된 오프라인
bundle에서 이를 표시하기만 하며 adapter·적용/검증 실행기·승인 서비스는 사용하거나 여기에 연결되지 않습니다. 기존의 고정된
...
이 값들은 향후 목표입니다. 라이브러리는 기준 목록과 대조하지 않고 제안된 변경 내용을 파싱해
확인하지도 않습니다. 기준 근거는 부분적일 수 있고 manifest는 제안 변경 중 일부만 다룰 수 있습니다.
