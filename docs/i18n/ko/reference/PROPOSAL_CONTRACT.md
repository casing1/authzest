...
리포트 스키마 `1.2`, 패키지 버전 `0.1.0a3`와 별개입니다. alpha.2 이후 도입한 이 순수 모듈은
이제 공개된 [v0.1.0-alpha.3](https://github.com/casing1/authzest/releases/tag/v0.1.0-alpha.3)의
fixture 명령에 사용됩니다. 최초 #46 계약 변경 자체에는 새 제품 CLI/API 명령이나 스캔 동작 변경이
없었습니다. 아래 데모 스크립트는 계속 소스 checkout이 필요하며 native CLI 명령이 아닙니다.
...
별도의 [#54 런타임 검사](../guides/RUNTIME_VERIFICATION.md)는 `--runtime-check`로 선택하며
선택은 승인이 아닙니다. 자체 정확한 계획의 별도 결정을 받은 뒤 동일한 번들 fixture만 실행하여
`app.debug`와 메모리 내 ASGI health 응답을 확인합니다. 임의 제안 내용을 실행하지 않고 순수 계약의
`not-run` 결과나 제안·결정 스키마도 바꾸지 않습니다.
...
향후 검증 의도이며 이미 존재하는 실행기나 실행을 허가하는 명령 목록이 아닙니다.
#54 실행기는 별도의 `owned-fixture-runtime-debug-health-v1` 검사 식별값을 사용하며 위 제안 식별값이나
모델이 제안한 테스트를 실행하지 않습니다. 계약은 shell 명령,
실행 파일, 인수, 환경, 설치 hook 필드를 받지 않습니다. 예상 결과와 설명도 비신뢰 데이터입니다.
후속 구현에서는 검토한 고정 구현으로 검사를 연결하고 별도의 실행 승인을 받아야 합니다.

별도의 [#61 예상 결과 manifest](EXPECTATION_CONTRACT.md)는 alpha.3 이후 추가한 미출시 순수 구조화
계약으로, 호출자가 작성한 정책·선언 목표를 제안과 기준 근거에 연결합니다. 기존 자유 서술형 예상 결과를
대체하거나 제안 스키마 `1.0`을 바꾸지 않습니다. 미리보기는 항상 draft/not-run이며 관찰 결과는 없고
인가 판정은 unknown뿐입니다. 기존 제안 결정·적용/검증 실행기는 이 manifest를 사용하거나 여기에 연결되지 않으며
테스트 생성과 실행은 별도 작업입니다.
미출시 [#63 CLI 미리보기](../guides/PROPOSAL_PREVIEW.md)는 엄격한 오프라인 bundle에서 연결된 산출물을
표시합니다. 이 읽기 전용 표시는 승인이나 실행 권한을 부여하지 않습니다.
...
참고하세요. 고정 fixture 런타임 단계도 별도의 범위 안내를 갖습니다. 이러한 제한된 결과만으로
더 넓은 #35 완료 조건을 충족하거나 릴리스를 승인하지 않습니다.
