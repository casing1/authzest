...
별도 [소스 전용 `proposal-check`](PROPOSAL_CHECK.md) 명령은 같은 bundle의 지원 범위 내
소스 snapshot을 선언 목표와 비교합니다. 이 미리보기의 draft/not-run 상태를 바꾸거나
정책 의도를 평가하거나 승인·실행 권한을 부여하지 않습니다.
