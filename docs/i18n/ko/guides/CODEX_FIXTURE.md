...
사용합니다. 일반 저장소 AI 검토, 임의 패치와 런타임 검증은 이 명령에서 구현하지 않습니다.
#52는 아래 설명하는 적용된 fixture 복사본의 선택적·별도 승인 소스 설정 검사만 추가하며,
모델을 추가로 호출하지 않습니다.
...
입력해야 하며 거절·취소하면 초안을 적용하지 않습니다. 적용 후 소스 설정 검사는 선택 사항이며
별도 승인이 필요합니다. 복구에는 `restore <proposal_id>`가 필요하고
...
워크플로는 `verification_scope: source-configuration`과 `passed`, `failed`, `not-run` 중 하나인
`verification_status`를 보고하며 `runtime_verification_status`는 항상 `not-run`입니다. 정상적으로
...
기존 `scripts.demo_apply` 흐름은 바뀌지 않고 `not-run`을 유지합니다. 이 소스 전용 추가 기능으로
...
기록이 아닙니다. 소스·테스트 실행, 보안 수정 효과 검증, 일반 저장소 AI와 남은 #35 작업은
아직 구현하지 않았습니다.
...
뜻하지 않습니다. #52 소스 설정 검사의 완료 조건은 충족했지만 런타임 검증은 `not-run`이며
#35는 계속 열어둡니다. 새 릴리스나 검증된 보안 수정을 주장하지 않습니다.
