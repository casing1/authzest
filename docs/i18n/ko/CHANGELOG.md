...
- #50의 소스 전용 [Codex fixture 명령](guides/CODEX_FIXTURE.md)을 추가했습니다. App Server 0.153.0
  ...
  새 POSIX 복사본의 별도 적용·복구 결정으로 연결합니다. 실제 검증은 대기 중이며 일반 저장소 AI와
  검증 실행은 미구현입니다. 기본 스캔은 오프라인을 유지하고 상위 #35는 열어둡니다. 새 릴리스는 없습니다.
- 새 턴을 요청하지 않고 엄격히 검증한 같은 턴의 Codex 스트림 복구 알림을 처리합니다.
  ...
  알림 개수는 제공자 시도나 과금의 상한이 아니며 실제 종단간 검증은 여전히 대기 중입니다.
