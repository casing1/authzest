...
<!-- 라벨을 지정하고 다른 담당자를 지우지 않은 채 casing1을 포함하며 연결된 이슈의 마일스톤과 맞춥니다.
여러 마일스톤에 공통인 로드맵/운영 작업이라면 마일스톤이 적용되지 않는 이유를 여기에 적습니다.
실제 메타데이터를 재조회하세요. 이 템플릿이 GitHub 필드를 자동으로 설정하지는 않습니다. -->
...
- [ ] 기존 메타데이터를 보존하면서 이슈와 PR에 적절한 라벨과 담당자 `casing1`을 지정했습니다.
- [ ] 연결된 이슈와 마일스톤을 맞추거나 명시적인 공통 작업 예외 사유를 기록하고 실제 값을 확인했습니다.
