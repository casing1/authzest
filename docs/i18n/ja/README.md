...
- 明示的な [`--runtime-check`](../../guides/RUNTIME_VERIFICATION.md) は固定した所有 fixture のみを対象とし、
  ...
  オプションの選択自体は実行承認ではありません。#54 の限定的な実ソース環境の受け入れ検証は合格し、最終 PR・リリースゲートは保留中です
...
行っておらず、追加機能は alpha.2 に含まれません。#52 は適用と復元の間に、別途承認する
...
限定的な実ソース環境の受け入れ検証は合格し、最終 PR・リリースゲートは保留中です。
一般リポジトリの AI レビューと #35 全体のフローは未完成で、新しいリリースを主張するものではありません。
